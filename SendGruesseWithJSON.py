...
def send_greeting(yeahboy: Any, amount: int, best_friend: bool = False) -> None:
    ...
    actions.perform()
    time.sleep(TIMER)
    actions_click_and_perform(DRIVER.find_element_by_css_selector('.button:nth-child(11) > span')) #Select Send Greeting
    time.sleep(TIMER)
    if amount == 50 and best_friend == True:
        actions_click_and_perform(DRIVER.find_element_by_css_selector("tr:nth-child(4) input")) #Select 50 G
    elif amount == 50 and best_friend != True:
        actions_click_and_perform(DRIVER.find_element_by_css_selector("tr:nth-child(2) input")) #Select 50 G
    else:
        actions_click_and_perform(DRIVER.find_element_by_css_selector("tr:nth-child(1) input")) #Select 1 G
    time.sleep(TIMER)
    actions_click_and_perform(DRIVER.find_element_by_css_selector("#assign_button > span")) #Send
    time.sleep(TIMER)
...
def use_girl(json_nummber: str, to_model: int) -> None:
    ...
    try:
        ...
        pass
    DRIVER.find_element_by_xpath("//div[@id='framework_all']/div/div[2]/div[2]/a[4]/div").click()
    DRIVER.execute_script(script="window.scrollTo(0,1000)")
    gButtons = select_model(credentials["accounts"][to_model])
    ...
    while True:
        ...
        if json_nummber == 1 and greetings_started-greetings >= greetings_started/2 and switched != True :
            if credentials["accounts"].index(credentials["accounts"][to_model]) == 0:
                gButtons = select_model(credentials["accounts"][1])
                name = credentials["accounts"][1]
            else:
                gButtons = select_model(credentials["accounts"][0])
                name = credentials["accounts"][0]
            switched = True
        if (greetings >= 50):
            send_greeting(gButtons, 50)
            print(f"Send 50 Greetings to {name}, {greetings-50} Grueße to go :D")
            continue
        elif (greetings >= 50 and json_nummber in [2,3]):
            send_greeting(gButtons, 50, True)
            print(f"Send 50 Greetings to {name}, {greetings-50} Grueße to go :D")
            continue
        elif (greetings > 0):
            send_greeting(gButtons, 1)
